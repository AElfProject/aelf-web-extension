...
export default class Background {
  ...
  static login(sendResponse, loginInfo) {
    this.checkSeed({ sendResponse }, ({ nightElfObject }) => {
      ...
      if (appPermissons.permissions.length) {
        ...
        const keypairLoggedIn = keypairs.find((item) => item.address === addressBinded);
  ...
  static async callAelfChain(sendResponse, callInfo) {
    ...
    if (dappAelfMeta) {
      ...
      sendResponse({
        ...errorHandler(
          400001,
          'Please connect the chain at first. ' +
  ...
  static async initCrossInstance(sendResponse, crossInfo) {
    ...
    aelfCrossMeta[`${address}${CROSS_INFO.from.id}${CROSS_INFO.to.id}`] = new CrossChainMethodsExtension({
      wallet,
      CROSS_INFO,
    });
  ...
  static callAelfContract(sendResponse, contractInfo, checkWhitelist = true, readonly = false, signedTx = false) {
    this.checkSeed({ sendResponse }, ({ nightElfObject }) => {
      ...
      if (checkWhitelist) {
        ...
        if (
          appPermissions.permissions.length &&
          !contractWhitelistCheck({
            sendResponse,
            appPermissions,
            contractAddress,
  ...
  static checkTimingLock(sendResponse) {
    if (pageState.inactivityInterval === 0) {
      return false;
    }
    ...
    if (pageState.seed && pageState.nightElf) {
      ...
      Background.setPageState({ timeoutLocker });
    }
  }
  ...
  static async checkInactivityInterval(sendResponse) {
  ...
  static setWhitelist(sendResponse, whitelistInput) {
    ...
    this.checkSeed({ sendResponse }, ({ nightElfObject }) => {
      ...
      const { contractAddress, whitelist } = payload;
  ...
  static removeContractOfPermission(sendResponse, removeInfo) {
    // {
    //     "appName": "hzzTest",
    //     "domain": "OnlyForTest!!!",
  ...
  static checkSeed(options, callback) {
    ...
    if (typeof sendResponse === 'function') {
      apis.storage.local.get(['nightElfEncrypto'], (result) => {
        console.log(result, 'result===checkSeed');
  ...
  static getSignature(sendResponse, options) {
    this.checkSeed(
      ...
      ({ nightElfObject }) => {
        ...
        const { address, hexToBeSign } = options;
